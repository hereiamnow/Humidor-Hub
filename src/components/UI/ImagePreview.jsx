 ...
 * @path src/components/UI/ImagePreview.jsx
 * @project Humidor Hub
 ...
 *
 * Displays the main image placeholder on the form (display only version).
 * Shows the image at a saved position, or a placeholder if no image is present.
 * Provides an overlay for editing or adding the image.
 *
...
const ImagePreview = ({ image, position, onClick }) => {
    console.log('ImagePreview props:', { image, position, onClick });
        ...
        <div className="group relative w-full h-64 border-1 bg-slate-50 transition-colors hover:border-slate-400 hover:bg-slate-100 overflow-hidden">
            {image ? (
                <>
                    ...
                    {console.log('Rendering image with position:', position)}
                    <img
                        src={image}
                        alt="Item Preview"
                ...
                <div
                    ...
                    className="flex h-full w-full flex-col items-center justify-center text-slate-500 cursor-pointer"
                >
